...
const numeradorCarrito = document.getElementById("numeradorCarrito");
...
const paintTotal = ({ cart, total }) => {
  let totalPrice = 0;

  cart.forEach(({ price, discount, Cart }) => {
    const priceWithDiscount = discount
      ? price - (price * discount) / 100
      : price;
    totalPrice += priceWithDiscount * Cart.amount;
  });

  outputTotal.textContent = convertFormatPeso(totalPrice);
};
...
window.addEventListener("load", async () => {
  try {
    ...
    /* numeradorCarrito.textContent = totalProducts; */
